...
export class FileDownloadLinkComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (hasValue(this.bitstream)) {
      ...
      this.secureViewerUrl$ = this.fileService.retrieveFileDownloadLink(bitstreamUrl).pipe(
        map(signedUrl => `/assets/secure-viewer/viewer.html?url=${encodeURIComponent('http://localhost:9000/viewer?url=' + signedUrl)}`)
      );
    }
  }
  get bitstreamHref(): string | null {
  if (!this.bitstream) return null;
  ...
  openPDFWithoutToolbar(): void {
    ...
    this.fileService.retrieveFileDownloadLink(href).subscribe(signedUrl => {
      ...
      const previewUrl = `http://localhost:9000/viewer?url=${encodeURIComponent(signedUrl)}#toolbar=0&navpanes=0&scrollbar=0`;
  ...
  DownloadLCP(): void {
    ...
    this.fileService.retrieveFileDownloadLink(href).subscribe(signedUrl => {
      // Gọi FastAPI viewer, giữ các query params và disable toolbar
      const signedUrlForLinux = signedUrl.replace('localhost', 'host.docker.internal');
      const previewUrl = `http://localhost:9000/viewer?url=${encodeURIComponent(signedUrlForLinux)}#toolbar=0&navpanes=0&scrollbar=0`;
      window.open(previewUrl, '_blank');
    });
  }
